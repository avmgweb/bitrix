...
$(function()
	...
	$(document)
		.on("vclick",   '.av-form-input-corp',       function() {$(this).parent().find('input').focus()})
		.on("focus",    '.av-form-input-corp input', function() {$(this).parent().addClass("active")   .behaviorFormElememtInputAvCorp("on")})
		.on("focusout", '.av-form-input-corp input', function() {$(this).parent().removeClass("active").behaviorFormElememtInputAvCorp()});
	});